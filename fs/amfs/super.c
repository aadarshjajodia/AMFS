...
void amfs_free_pattern_list(struct pattern_list *pattern_list)
{
	struct pattern_list *tmp;
	struct list_head *pos, *q;

	list_for_each_safe(pos, q, &pattern_list->list) {
		tmp = list_entry(pos, struct pattern_list, list);
		list_del(pos);
		/*kfree(tmp->pattern_data);*/
		kfree(tmp);
	}
	kfree(pattern_list);
}
static void amfs_put_super(struct super_block *sb)
{
	...
	version_number = *AMFS_SB(sb)->pattern_version_number;

	fp = validatePatternFile(AMFS_SB(sb)->pattern_file_name);
	if (!IS_ERR(fp)) {
		fp->f_inode->i_op->setxattr(fp->f_path.dentry,
						AMFS_ATTR_PATTERN_FILE_VERSION_NUMBER,
						&version_number, sizeof(version_number), 0);
		filp_close(fp, NULL);
	}

	/* Freeing the allocated list */
	amfs_free_pattern_list(AMFS_SB(sb)->patt_list);

	/* decrement lower super references */
	s = amfs_lower_super(sb);
	amfs_set_lower_super(sb, NULL, NULL, NULL, NULL);
